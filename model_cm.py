...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
                 ...
                 use_speaker=True, use_modal=False, norm='LN2', num_L = 3, num_K = 4, print_info=False):
        
        super(Model, self).__init__()
        # self.gate_fusion = gate_fusion
        # self.dyweight_fusion = dyweight_fusion
        ...
        if self.graph_type=='hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        ...
                                        use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K
                                        )#gate_fusion= self.gate_fusion, dyweight_fusion = self.gate_fusion
            if print_info:
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch, True)
            emotions_feat_clone = emotions_feat
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)
        else:
            print("There are no such kind of graph")        
        return log_prob, edge_index, edge_norm, edge_type, edge_index_lengths, emotions_feat_clone



class Model_Intra(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
        ...
        super(Model_Intra, self).__init__()
        # self.gate_fusion = gate_fusion
        # self.dyweight_fusion = dyweight_fusion
        self.base_model = base_model
        self.avec = avec
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch, mask_context='intra')
            emotions_feat_clone = emotions_feat
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)
...
class Model_Inter(nn.Module):
    ...
    def __init__(self,base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future,
        ...
        super(Model_Inter, self).__init__()
        # self.gate_fusion = gate_fusion
        # self.dyweight_fusion = dyweight_fusion
        self.base_model = base_model
        self.avec = avec
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat = self.graph_model(features_a, features_v, features_l, seq_lengths, qmask, epoch, mask_context="inter")
            emotions_feat_clone = emotions_feat
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            log_prob = F.log_softmax(self.smax_fc(emotions_feat), 1)
